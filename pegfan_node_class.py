...
def train(datastr,splitstr):
    adj_raw, adj, adj_i, features, labels, idx_train, idx_val, idx_test, num_features, num_labels = full_load_data(datastr,splitstr)
    features = features.to(device)
    framelets_list, framelets_T_list = get_spatial_framelets_list(np.asarray(adj_raw), datastr, args.h)
...
for i in range(10):
    ...
    accuracy_data, best_val = train(datastr,splitstr)
...
""" 
The following lines are adapted from 
https://github.com/CUAI/Non-Homophily-Large-Scale
"""

print("Training time: {:.4f}s".format(training_time))
print(f"Test accuracy: {np.mean(acc_list)}, {np.round(np.std(acc_list),2)}")

filename = f'results/{args.data}' + '_' + args.type + '_' + str(args.h) + '.csv' 
print(f"Saving results to {filename}")
with open(f"{filename}", 'a+') as write_obj:
    write_obj.write(f"{np.mean(val_list):.3f} ± {np.std(val_list):.3f}," +
