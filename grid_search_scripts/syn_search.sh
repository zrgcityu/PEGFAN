...
# use 00, 02, 04, 06, 08, 10 to specify gamma
            ...
            for lr_sca in "${lr_sca_lst[@]}"; do
                for lr_fc in "${lr_fc_lst[@]}"; do
                    for dropout in "${dropout_lst[@]}"; do
                        echo "Running"
                        python syn_pegfan_node_class.py  --feat_type heterophily --layer 3 --w_att $wd_sca --w_fc2 $wd_fc2 --w_fc1 $wd_fc1 --dropout $dropout --lr_fc $lr_fc --lr_att $lr_sca --layer_norm 1 --dev 0 --type b --gamma 00 --table 1
                    done 
                done 
            done
        done
